...
}for(var i in h){if(h.hasOwnProperty(i)){d[i]=h[i];}}};b.init=function(h){if(h===null){h=c;}var i=h.onload||function(){};h.onload=function(){var j=document.getElementsByTagName("img"),k=[],l,n;
for(l=0;l<j.length;l+=1){n=j[l];var o=n.src,m=o.substr(o.lastIndexOf(".")+1);if(!!!n.getAttributeNode("data-no-retina")&&m!=="svg"){k.push(new e(n));}}i();
...
function createGraph() {
  ...
  function addLink(fromId, toId, data) {
    ...
    if (_.isNull(getLink(fromId, toId)) && _.isNull(getLink(toId, fromId))) {
      ...
      if (_.isUndefined(getLink(fromId, toId))){
        link = getLink(toId, fromId);
      } else {
        link = getLink(fromId, toId);
      }
    }
  ...
  function resetLinkVisited(){
    forEachLink(function(link){
      link.data.__visited = false;
    });
  }

  function findRoutes(start, finish) {
    var path = [];
    
    // var start_node = getNode(start);
    resetLinkVisited();

    path.push(start);

    var undo_visit_links = [];
    ...
    function visit(node_id){
      ...
      var availableLinks = _.filter(node.links, function(link){
        return link.data.__visited == false;
      });

      if (availableLinks.length == 0) {
      // debugger;
        // 더이상 갈 데가 없으므로 돌아 나와야 함.
        var poped = path.pop();
        // var d = "";
        // _.each(path, function(p){ d += " " + p; });
        // console.log("current_path: " + d);


        visit(path[path.length - 1]);

      } else {
        var link = availableLinks[Math.floor(randomBetween(0, availableLinks.length))];
        
        link.data.__visited = true;



        var beStackedNode = link.fromId == node_id ? link.toId : link.fromId;
        path.push(beStackedNode);
    ...
    _.each(path, function(vertex, i){
      if (i + 1 <= path.length - 1) {
        var link = getLink(path[i + 1], vertex) || getLink(vertex, path[i + 1]);
        link.data.__visited = true;
      }
    });
    // debugger;
...
WY.models = WY.models || {};
WY.views = WY.views || {};
WY.constants = WY.constants || {};
WY.dispatcher = WY.dispatcher || _.extend({}, Backbone.Events);
